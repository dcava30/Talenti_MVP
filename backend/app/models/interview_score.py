...
class InterviewScore(Base):
    ...
    # Culture fit score (0-100) — from model-service-1, canonical behavioural dimensions
    culture_fit_score: Mapped[int] = mapped_column(Integer)

    # Skills score (0-100) and outcome (PASS | REVIEW | FAIL) — from model-service-2, JD-driven competencies
    skills_score: Mapped[int | None] = mapped_column(Integer, nullable=True)
    skills_outcome: Mapped[str | None] = mapped_column(String, nullable=True)
