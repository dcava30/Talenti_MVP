...
type InterviewStatus = "connecting" | "greeting" | "questioning" | "listening" | "processing" | "completed" | "error";
...
const LiveInterview = () => {
  ...
  const [antiCheatSignals, setAntiCheatSignals] = useState<AntiCheatSignal[]>([]);
  const [connectionStatus, setConnectionStatus] = useState<"connected" | "reconnecting" | "disconnected">("connected");
  const [silenceSeconds, setSilenceSeconds] = useState(0);
  const [applicationId, setApplicationId] = useState<string | null>(null);
  const [competenciesCovered, setCompetenciesCovered] = useState<string[]>([]);
  ...
  const [remoteVideoStreams, setRemoteVideoStreams] = useState<Array<{
  ...
  const lastSpeechRef = useRef<Date>(new Date());
  ...
  const {
    callState,
    isInitialized: isAcsInitialized,
    isMuted,
    isVideoOn,
    error: acsError,
    remoteParticipants,
    ...
    joinCall,
    hangup: acsHangup,
    toggleMute: acsToggleMute,
    toggleVideo: acsToggleVideo,
    getDeviceManager,
    cleanup: cleanupAcs,
  } = useAcsCall({
    ...
    onParticipantLeft: (participant) => {
      setRemoteVideoStreams(prev => 
        prev.filter(s => s.participantId !== participant.identifier.toString())
      );
    },
  });
  ...
  const {
    ...
    isSpeaking: azureIsSpeaking,
    transcript: azureTranscript,
    interimTranscript: azureInterimTranscript,
    isInitialized: azureSpeechInitialized,
    error: azureSpeechError,
    initialize: initializeAzureSpeech,
    startListening: azureStartListening,
    stopListening: azureStopListening,
    speak: azureSpeak,
    stopSpeaking: azureStopSpeaking,
    resetTranscript: azureResetTranscript,
  } = useAzureSpeech({
    voice: "en-AU-NatashaNeural",
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        const newSignal: AntiCheatSignal = {
          type: "tab_switch",
          timestamp: new Date(),
        };
        ...
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {useAzureSpeechMode && !useAvatarMode && (
                <Badge variant="outline" className="flex items-center gap-1 text-primary border-primary/50">
      ...
      <div className="flex-1 container mx-auto px-4 py-6 max-w-6xl flex flex-col lg:flex-row gap-6">
        ...
        <div className="lg:w-1/3 space-y-4">
          ...
          <AvatarRenderer
            ...
            audioStream={avatarAudioStream}
            isConnecting={avatarConnecting}
            isSpeaking={avatarSpeaking || isSpeaking}
            fallbackName="AI Interviewer"
            className="aspect-video"
          />

          {/* Local Video (ACS mode) */}
          {useAcsMode && localVideoStream && (
          ...
          {/* Remote Videos */}
          {useAcsMode && remoteVideoStreams.length > 0 && (
            <div className="space-y-2">
              {remoteVideoStreams.map((remote) => (
                <VideoTile
              ...
              <div className="border-t border-border pt-4 space-y-3">
                <div className="min-h-[60px] p-3 bg-accent/50 rounded-lg">
                  <p className="text-sm">
                    ...
                    {!transcript && !interimTranscript && (
                      <span className="text-muted-foreground">Speak your answer...</span>
                    )}
                  </p>
                </div>
                ...
                <div className="flex items-center justify-center gap-4">
                  <div className="flex-1 h-2 bg-accent rounded-full overflow-hidden">
                    <div 
                      className={`h-full bg-primary transition-all ${isListening ? "animate-pulse" : ""}`}
                      style={{ width: isListening ? "60%" : "0%" }}
                    />
                  </div>
                  {transcript && silenceSeconds > 0 && (
          ...
          <div className="mt-6">
            {useAcsMode ? (
              <CallControls
                isMuted={effectiveIsMuted}
                isVideoOn={isVideoOn}
                isConnected={isAcsInitialized || status !== "connecting"}
              ...
              <div className="flex items-center justify-center gap-4">
                <Button
                  ...
                  disabled={status === "connecting" || status === "completed" || status === "processing"}
                >
                  {effectiveIsMuted ? (
