...
const EditRoleRubric = () => {
    ...
    const organisation = orgUser?.organisation;
    ...
    const updateWeight = (index, value) => {
        setDimensions(prev => prev.map((d, i) => (i === index ? { ...d, weight: value } : d)));
    };
      ...
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        ...
        <Card className={`p-4 mb-6 ${isValid ? "bg-green-500/10 border-green-500/20" : "bg-destructive/10 border-destructive/20"}`}>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {!isValid && (<span className="text-sm text-destructive">Must equal 100%</span>)}
            </div>
          </div>
        </Card>
        ...
        <Card className="p-6 mb-6">
          ...
          <div className="space-y-6">
            {dimensions.map((dim, index) => (<div key={dim.dimension} className="space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                  ...
                  <div className="flex items-center gap-2">
                    <Input type="number" className="w-20 text-right" value={dim.weight} onChange={(e) => updateWeight(index, parseInt(e.target.value) || 0)} min={0} max={100}/>
                    <span className="text-sm text-muted-foreground">%</span>
                  </div>
                </div>
                <Slider value={[dim.weight]} onValueChange={([value]) => updateWeight(index, value)} max={100} step={5}/>
              </div>))}
          </div>
        </Card>
