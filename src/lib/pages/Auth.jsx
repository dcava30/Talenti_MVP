...
const Auth = () => {
    ...
    useEffect(() => {
        const checkUserAndRedirect = async (selectedUserType) => {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 flex items-center justify-center p-4">
      <Card className="w-full max-w-md p-8">
        ...
        <div className="flex gap-4 mb-8">
          <button type="button" onClick={() => setUserType("candidate")} className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-lg border-2 transition-all ${userType === "candidate"
          ...
          <button type="button" onClick={() => setUserType("organisation")} className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-lg border-2 transition-all ${userType === "organisation"
            ? "border-primary bg-primary/5"
            : "border-border hover:border-primary/50"}`}>
          ...
          <TabsContent value="signup">
            <form onSubmit={handleSignUp} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground"/>
                  <Input id="confirm-password" type={showPassword ? "text" : "password"} placeholder="••••••••" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="pl-10" required/>
                </div>
