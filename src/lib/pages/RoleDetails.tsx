...
const RoleDetails = () => {
  ...
  const selectedApplicationsForComparison = applications?.filter((app) =>
  ...
  const handleDownloadPDF = async (interviewId: string) => {
    if (!role || !orgData?.organisation) return;
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          {role.description && (
            <p className="text-muted-foreground mb-4 max-w-2xl">{role.description}</p>
          )}

          <div className="flex flex-wrap gap-3">
            <Button 
              onClick={handleGenerateShortlist}
              disabled={isGenerating || !applications?.length}
        ...
        {/* AI Shortlist View */}
        {showShortlist && shortlistData && (
          <ShortlistView 
            matches={shortlistData.matches}
            onClose={() => {
        ...
        {/* Comparison View */}
        {showComparison && selectedApplicationsForComparison.length >= 2 && (
          <div className="mb-6">
            <CandidateComparison
              selectedApplications={selectedApplicationsForComparison}
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-4">
              ...
              {sortedApplications?.some(a => a.match_score !== null) && (
                  ...
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-start gap-4 flex-1">
                        <Checkbox
                          checked={isSelected}
                          onCheckedChange={() => toggleCandidateSelection(application.id)}
                          className="mt-1"
                        />
                    ...
                    {interview?.status === "completed" && (
                      <div className="flex gap-2">
                        ...
                        <Button 
                          size="sm" 
                          variant="outline"
                          disabled={downloadingPDF === interview.id}
                          onClick={() => handleDownloadPDF(interview.id)}
                        >
                          {downloadingPDF === interview.id ? (
                            <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                          ) : (
      ...
      {role && orgData?.organisation && (
        <SendInvitationDialog
          ...
          companyName={orgData.organisation.name}
          onSuccess={() => {
            queryClient.invalidateQueries({ queryKey: ["role-applications", roleId] });
          }}
        />
