...
import SkillsAssessmentSummaryCard from "@/components/SkillsAssessmentSummaryCard";
...
import {
    isTdsRecruiterSkillsSummaryUiEnabled,
    loadInterviewSkillsAssessmentSummary,
} from "@/lib/tdsRecruiterSkillsSummaryUi";
const formatTime = (ms) => {
...
const InterviewReport = () => {
    ...
    const recruiterSkillsSummaryUiEnabled = isTdsRecruiterSkillsSummaryUiEnabled();
    const legacySkillsUiVisible = !recruiterSkillsSummaryUiEnabled;
    ...
    const [skillsSummaryStatus, setSkillsSummaryStatus] = useState(recruiterSkillsSummaryUiEnabled ? "loading" : "disabled");
    ...
    useEffect(() => {
        if (interviewId) {
            ...
            if (recruiterSkillsSummaryUiEnabled) {
                loadSkillsSummaryData();
            }
            else {
                setSkillsSummary(null);
                setSkillsSummaryStatus("disabled");
            }
        }
    }, [interviewId, recruiterDecisionUiEnabled, recruiterSkillsSummaryUiEnabled]);
    ...
    const loadSkillsSummaryData = async () => {
        if (!interviewId || !recruiterSkillsSummaryUiEnabled)
            return;
        setSkillsSummaryStatus("loading");
        const result = await loadInterviewSkillsAssessmentSummary(interviewId, {
            enabled: recruiterSkillsSummaryUiEnabled,
            api: interviewsApi,
        });
        setSkillsSummary(result.summary);
        setSkillsSummaryStatus(result.status);
    };
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            const success = await downloadInterviewReport(interview.id, interview.application.job_role.title, interview.application.job_role.organisation.name);
            if (success) {
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex gap-6 text-sm text-muted-foreground">
            {interview.started_at && (<div className="flex items-center gap-1">
                <Clock className="w-4 h-4"/>
                {new Date(interview.started_at).toLocaleDateString()}
              </div>)}
            {interview.duration_seconds && (<div className="flex items-center gap-1">
                <Clock className="w-4 h-4"/>
                Duration: {formatDuration(interview.duration_seconds)}
              </div>)}
            {interview.recording_url && (<Button variant="link" size="sm" className="h-auto p-0" asChild>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {recruiterSkillsSummaryUiEnabled && (<SkillsAssessmentSummaryCard summary={skillsSummary} status={skillsSummaryStatus}/>)}
            <Tabs defaultValue="summary" className="w-full">
              <TabsList className={`grid w-full ${legacySkillsUiVisible ? "grid-cols-4" : "grid-cols-3"}`}>
                <TabsTrigger value="summary">Summary</TabsTrigger>
                <TabsTrigger value="dimensions">{recruiterDecisionUiEnabled ? "Legacy Score Details" : "Score Details"}</TabsTrigger>
                {legacySkillsUiVisible && (<TabsTrigger value="skills">{recruiterDecisionUiEnabled ? "Legacy Skills Fit" : "Skills Fit"}</TabsTrigger>)}
                <TabsTrigger value="transcript">Transcript</TabsTrigger>
              </TabsList>

              <TabsContent value="summary" className="space-y-6 mt-6">
                {score?.narrative_summary && (<Card className="p-6">
                ...
                {score?.candidate_feedback && (<Card className="p-6">
                    <h3 className="font-semibold mb-4 flex items-center gap-2">
                ...
                {score?.human_override && (<Card className="p-6 border-primary/50 bg-primary/5">
                ...
                <Card className="p-6">
                  ...
                  <div className="space-y-6">
                    {dimensions.map((dim) => (<div key={dim.id} className="space-y-3">
                        ...
                        {dim.evidence && (<p className="text-sm text-muted-foreground">{dim.evidence}</p>)}
                        {dim.cited_quotes && Array.isArray(dim.cited_quotes) && dim.cited_quotes.length > 0 && (<div className="mt-2 space-y-2">
                            <p className="text-xs font-medium text-muted-foreground">Supporting Quotes:</p>
                            {dim.cited_quotes.map((quote, idx) => (<blockquote key={idx} className="border-l-2 border-primary/50 pl-3 text-sm italic text-muted-foreground">
                                "{quote}"
                              </blockquote>))}
                          </div>)}
                        <Separator />
                    ...
                    {dimensions.length === 0 && (<p className="text-center text-muted-foreground py-8">
              ...
              {legacySkillsUiVisible && (<TabsContent value="skills" className="mt-6">
                  {score?.service2_raw ? (
            ...
            return (<div key={segment.id} className={`flex gap-3 ${isAI ? "" : "flex-row-reverse"}`}>
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${isAI ? "bg-primary text-primary-foreground" : "bg-secondary"}`}>
                    ...
                    : "bg-secondary text-foreground"}`}>
                      ...
                      {transcripts.length === 0 && (<p className="text-center text-muted-foreground py-8">
          ...
          <div className="space-y-6">
            ...
            <Card className="p-6">
              ...
              {score ? (<div className="text-center">
                  {isEditing ? (<div className="space-y-4">
                      <Input type="number" min="0" max="100" value={editedScore ?? ""} onChange={(e) => setEditedScore(Number(e.target.value))} className="text-center text-2xl font-bold"/>
                    ...
                    setEditedScore(score.overall_score);
                        ...
                        <Button size="sm" className="flex-1" onClick={() => setShowOverrideDialog(true)}>
                    ...
                    : "N/A"}
                      ...
                      {score.human_override && (<Badge variant="outline" className="mt-2">
                          Human Override
                        </Badge>)}
                    </>)}
              ...
              {score?.model_version && (<p className="text-xs text-muted-foreground text-center mt-4">
            ...
            {/* Skills Fit Summary */}
            {legacySkillsUiVisible && score?.service2_raw && (
              <Card className="p-6">
            ...
            {score?.anti_cheat_risk_level && (<Card className="p-6">
                ...
                interview.anti_cheat_signals.length > 0 && (<div className="mt-2">
                        <p className="text-sm text-muted-foreground mb-2">Signals:</p>
                        <ul className="text-sm space-y-1">
                          {interview.anti_cheat_signals.map((signal, idx) => (<li key={idx} className="text-muted-foreground">
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea id="reason" placeholder="Explain why you're overriding the AI score..." value={overrideReason} onChange={(e) => setOverrideReason(e.target.value)} rows={4}/>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowOverrideDialog(false)} disabled={isSaving}>
              Cancel
            </Button>
