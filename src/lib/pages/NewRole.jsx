...
const NewRole = () => {
    ...
    const updateWeight = (index, value) => {
        setScoringWeights(prev => prev.map((w, i) => i === index ? { ...w, weight: value } : w));
    };
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="title" placeholder="e.g., Senior Frontend Developer" value={title} onChange={(e) => setTitle(e.target.value)} required/>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="location" placeholder="e.g., Sydney, Australia" value={location} onChange={(e) => setLocation(e.target.value)}/>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="salaryMin" type="number" placeholder="e.g., 100000" value={salaryMin} onChange={(e) => setSalaryMin(e.target.value)}/>
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="salaryMax">Salary Range (Max)</Label>
                  <Input id="salaryMax" type="number" placeholder="e.g., 150000" value={salaryMax} onChange={(e) => setSalaryMax(e.target.value)}/>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={handleExtractRequirements} disabled={isExtracting || !description.trim()}>
                    {isExtracting ? (<>
                ...
                <Textarea id="description" rows={8} placeholder="Describe the role, responsibilities, and requirements..." value={description} onChange={(e) => setDescription(e.target.value)} required/>
              </div>
          ...
          {extractedRequirements && (<Card className="p-6 border-primary/20 bg-primary/5">
              <div className="flex items-center gap-2 mb-4">
                <Sparkles className="w-5 h-5 text-primary"/>
                <h2 className="text-xl font-semibold">AI-Extracted Requirements</h2>
                <Button type="button" variant="ghost" size="sm" className="ml-auto" onClick={() => setExtractedRequirements(null)}>
                  <X className="w-4 h-4"/>
                </Button>
              </div>
              
              <div className="space-y-4">
                {extractedRequirements.skills.length > 0 && (<div>
                    <p className="text-sm font-medium mb-2">Skills</p>
                    <div className="flex flex-wrap gap-2">
                      {extractedRequirements.skills.map((skill, i) => (<Badge key={i} variant="secondary">{skill}</Badge>))}
                    </div>
                  </div>)}

                {extractedRequirements.experience.length > 0 && (<div>
                    <p className="text-sm font-medium mb-2">Experience</p>
                    <ul className="text-sm text-muted-foreground space-y-1">
                      {extractedRequirements.experience.map((exp, i) => (<li key={i} className="flex items-start gap-2">
                ...
                {extractedRequirements.responsibilities.length > 0 && (<div>
                    <p className="text-sm font-medium mb-2">Key Responsibilities</p>
                    <ul className="text-sm text-muted-foreground space-y-1">
                      {extractedRequirements.responsibilities.slice(0, 5).map((resp, i) => (<li key={i} className="flex items-start gap-2">
                          <Check className="w-4 h-4 text-primary mt-0.5 flex-shrink-0"/>
                          {resp}
                        </li>))}
                    </ul>
                ...
                {extractedRequirements.interviewQuestions.length > 0 && (<div>
                    <p className="text-sm font-medium mb-2">Suggested Interview Questions</p>
                    <ol className="text-sm text-muted-foreground space-y-1 list-decimal list-inside">
                      {extractedRequirements.interviewQuestions.map((q, i) => (<li key={i}>{q}</li>))}
                    </ol>
                  </div>)}
              </div>
            </Card>)}
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4">
              {scoringWeights.map((weight, index) => (<div key={weight.dimension} className="flex items-center justify-between p-3 rounded-lg border border-border">
                  <div>
                    <p className="font-medium">{weight.label}</p>
                    <p className="text-sm text-muted-foreground">{weight.description}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Input type="number" className="w-20" value={weight.weight} onChange={(e) => updateWeight(index, parseInt(e.target.value) || 0)} min="0" max="100"/>
              ...
              <div className="text-sm text-muted-foreground text-right">
                ...
                {scoringWeights.reduce((sum, w) => sum + w.weight, 0) !== 100 && (<span className="text-destructive ml-2">(should equal 100%)</span>)}
              </div>
            </div>
          </Card>
