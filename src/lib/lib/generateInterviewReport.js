...
export const fetchInterviewReportData = async (interviewId, roleTitle, companyName) => {
    try {
...
export const generateInterviewPDF = (data) => {
    const doc = new jsPDF();
    ...
    if (data.dimensions.length > 0) {
        ...
        data.dimensions.forEach((dim) => {
            ...
            if (dim.evidence) {
                ...
                evidenceLines.slice(0, 2).forEach((line) => {
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        doc.text(`Page ${i} of ${totalPages} | ${data.companyName} - ${data.roleTitle}`, margin, doc.internal.pageSize.getHeight() - 10);
    }
    // Download
    const filename = `interview-report-${data.candidateId.slice(0, 8)}-${new Date().toISOString().split("T")[0]}.pdf`;
    doc.save(filename);
};
export const downloadInterviewReport = async (interviewId, roleTitle, companyName) => {
    try {
