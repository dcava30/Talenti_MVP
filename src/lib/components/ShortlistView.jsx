    ...
    return (<Card className="p-6 mb-6 border-primary/20 bg-gradient-to-br from-primary/5 to-background">
      ...
      <ScrollArea className="h-[500px] pr-4">
        <div className="space-y-4">
          {matches.map((match, index) => (<Card key={match.applicationId} className="p-4 hover:bg-accent/50 transition-colors cursor-pointer" onClick={() => onSelectCandidate(match.applicationId)}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1 text-sm text-muted-foreground">
                      {match.location !== "Not specified" && (<span className="flex items-center gap-1">
                          <MapPin className="w-3 h-3"/>
                          {match.location}
                        </span>)}
                      {match.availability !== "Not specified" && (<span className="flex items-center gap-1">
                          <Clock className="w-3 h-3"/>
                          {match.availability}
                        </span>)}
                      {match.workMode !== "Not specified" && (<span className="flex items-center gap-1">
              ...
              {/* Skills */}
              {match.skills.length > 0 && match.skills[0] !== "Not specified" && (<div className="mb-3">
                  <p className="text-xs text-muted-foreground mb-1.5">Key Skills</p>
                  <div className="flex flex-wrap gap-1.5">
                    {match.skills.slice(0, 5).map((skill, i) => (<Badge key={i} variant="secondary" className="text-xs">
                        {skill}
                      </Badge>))}
                    {match.skills.length > 5 && (<Badge variant="outline" className="text-xs">
              ...
              <div className="space-y-1.5">
                ...
                {match.matchReasons.map((reason, i) => (<div key={i} className="flex items-start gap-2 text-sm">
              ...
              {/* Experience Preview */}
              {match.experience !== "Not specified" && match.experience !== "No experience listed" && (<div className="mt-3 pt-3 border-t border-border/50">
