...
export function AuditTrailViewer({ organisationId }: AuditTrailViewerProps) {
  ...
  const formatChangeDetails = (log: AuditLogEntry): string | null => {
    if (!log.old_values && !log.new_values) return null;
    
    if (log.action === "score_override" && log.new_values) {
      const newScore = (log.new_values as Record<string, unknown>).overall_score;
      const reason = (log.new_values as Record<string, unknown>).override_reason;
