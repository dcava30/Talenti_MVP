 ...
 * ```
 */
export const useAzureSpeech = (options = {}) => {
    ...
    const fetchToken = useCallback(async () => {
        try {
            ...
            const fallbackExpirySeconds = 9 * 60;
            tokenExpiryRef.current = Date.now() + ((data.expiresIn ?? fallbackExpirySeconds) * 1000);
            return { ...data, expiresIn: data.expiresIn ?? fallbackExpirySeconds };
        }
        catch (err) {
    ...
    const initialize = useCallback(async () => {
        try {
            ...
            speechConfigRef.current = SpeechSDK.SpeechConfig.fromAuthorizationToken(tokenData.token, tokenData.region);
            // Configure for low latency
            speechConfigRef.current.speechRecognitionLanguage = "en-US";
            speechConfigRef.current.speechSynthesisVoiceName = voice;
            speechConfigRef.current.setProperty(SpeechSDK.PropertyId.SpeechServiceConnection_InitialSilenceTimeoutMs, "5000");
            speechConfigRef.current.setProperty(SpeechSDK.PropertyId.SpeechServiceConnection_EndSilenceTimeoutMs, "1000");
    ...
    const startListening = useCallback(async () => {
        ...
        try {
            ...
            recognizerRef.current = new SpeechSDK.SpeechRecognizer(speechConfigRef.current, audioConfigRef.current);
    ...
    const speak = useCallback(async (text) => {
        ...
        try {
            ...
            synthesizerRef.current = new SpeechSDK.SpeechSynthesizer(speechConfigRef.current, audioOutput);
            setIsSpeaking(true);
            options.onTTSStart?.();
            synthesizerRef.current.speakTextAsync(text, (result) => {
