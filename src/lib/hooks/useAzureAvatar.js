 ...
 * ```
 */
export const useAzureAvatar = (options = {}) => {
    const { character = "lisa", style = "casual-sitting", voice = "en-AU-NatashaNeural", videoWidth = 1280, videoHeight = 720, onAvatarStarted, onAvatarStopped, onSpeakingStart, onSpeakingEnd, onError, } = options;
     ...
     */
    const fetchIceServerInfo = async (region, authToken) => {
        try {
    ...
    const initialize = useCallback(async () => {
        ...
        try {
            ...
            peerConnectionRef.current = new RTCPeerConnection({
                iceServers: [
                    {
                        urls: iceInfo.urls.filter((url) => url.startsWith("turn")),
            ...
            avatarSynthesizerRef.current = new SpeechSDK.AvatarSynthesizer(speechConfigRef.current, avatarConfig);
     ...
     * @returns true on success, false on failure
     */
    const speakSsml = useCallback(async (ssml) => {
        if (!avatarSynthesizerRef.current || !isInitialized) {
            console.warn("[AzureAvatar] Avatar not initialized");
            return false;
        }
