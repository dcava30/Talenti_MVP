...
- **Backend uses PostgreSQL** via `DATABASE_URL` defaults and Alembic config; no Postgres drivers in backend dependencies. See `backend/app/core/config.py`, `backend/alembic.ini`, and `backend/pyproject.toml`.-F:backend/app/core/config.py-L1-L35-F:backend/alembic.ini-L1-L3-F:backend/pyproject.toml-L1-L26-
...
- **Schema creation on cold start**: Alembic migration is executed at app startup, enabling cold-start schema creation (PostgreSQL).-F:backend/app/main.py-L28-L37-
- **PostgreSQL constraints**: FKs and unique indexes are defined in the initial migration; FK enforcement is explicitly enabled for PostgreSQL connections.-F:backend/alembic/versions/0001_initial.py-L12-L231-F:backend/app/db.py-L7-L23-
- **Validation gaps**:
  - Application creation does **not** validate existence of `job_role_id` or `candidate_profile_id`. This can create orphaned rows and breaks FK expectations in PostgreSQL until commit fails at runtime.-F:backend/app/api/applications.py-L18-L48-
  ...
  - Interview creation does **not** validate `application_id` existence before insert, leading to FK violations or silent errors depending on PostgreSQL settings.-F:backend/app/api/interviews.py-L45-L79-
...
- **DATABASE_URL** resolves to PostgreSQL only (backend + ACS service), no Postgres fallback present in runtime config or migrations.-F:backend/app/core/config.py-L1-L12-F:backend/alembic.ini-L1-L3-F:python-acs-service/app/config.py-L56-L69-
- **Startup scripts**: backend relies on Alembic upgrade during FastAPI startup; this is adequate for single-node PostgreSQL but risky for multi-process deployments (migration conflicts).-F:backend/app/main.py-L28-L37-
